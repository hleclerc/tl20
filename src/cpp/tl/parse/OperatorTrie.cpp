...
void OperatorTrie::init_default_operators() {
    ...
    ++curr_prio;
    register_symbol_operator( new OperatorData{ .str = "++" , .name = "operator ++" , .priority = curr_prio, .l2r = true, .max_rch = 0 } );
    register_symbol_operator( new OperatorData{ .str = "+"  , .name = "operator +"  , .priority = curr_prio, .l2r = true, .max_rch = 1 } );

    ++curr_prio;
    register_symbol_operator( new OperatorData{ .str = "*"  , .name = "operator *"  , .priority = curr_prio, .l2r = true, .max_rch = 1 } );

    ++curr_prio;
    register_letter_operator( new OperatorData{ .str = "and", .name = "operator and", .priority = curr_prio, .l2r = true, .max_rch = 1 } );
    register_letter_operator( new OperatorData{ .str = "or" , .name = "operator or" , .priority = curr_prio, .l2r = true, .max_rch = 1 } );
}
